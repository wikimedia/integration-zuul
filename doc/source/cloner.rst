...
``git clone`` will complain. This happens cloning a sub project before
its parent project. For example::

 zuul-cloner project/plugins/plugin1 project

Will create the directorty ``project`` when cloning the plugin. The
cloner process the clones in order so you should swap the projects::
